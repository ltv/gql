    ...
    enter(node) {
      ...
      concatProps.forEach(propName => {
        if (node[propName] && oldNode[propName]) {
          node[propName] = oldNode[propName].reduce((res, field) => {
            const has = res.find(f => f.name.value === field.name.value);
            if (!has) {
              res = [...res, field];
            }
            return res;
